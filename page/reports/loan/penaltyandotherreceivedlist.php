...
class page_reports_loan_penaltyandotherreceivedlist extends Page {
	public $title="Penalty & Other Received List";
	function init(){
		parent::init();
		$till_date="";
		if($_GET['to_date']){
		...
		$grid->setModel($transaction_row_model,array('AccountNumber','member_name','member_address','member_landmark','FatherName','PhoneNos','amountCr','Narration','created_at','dealer_name','is_in_legal','is_given_for_legal_process'));
		...
		if($form->isSubmitted()){
			...
			$grid->js()->reload(array('dealer'=>$form['dealer'],'from_date'=>$form['from_date']?:0,'to_date'=>$form['to_date']?:0,'loan_type'=>$form['loan_type'],'receive_type'=>$form['receive_type'],'legal_status'=>$form['legal_status'],'filter'=>1))->execute();
