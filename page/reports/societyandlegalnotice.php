		...
		if($_GET['filter']){
					...
					$account_model->addExpression('include')->set(function($m,$q){
						return $q->expr(
							'IF(
								...
								1,
								IF(
									[created_at]<"2018-04-01" AND [due_premium_count] = 3 AND [last_premium_field] >= "[last_premium_value_3_month]" AND [last_paid_date_field] < "[last_paid_date_value_3_month]" AND [paid_premium_count] <= [paid_premium_count_value] AND ([society_notice_sent_on] < "[today_minus_60_days]" OR [society_notice_sent_on] is null) AND [recent_due_premium_date] <= "[today_minus_15_days]" AND [bike_surrendered]=0 AND [is_given_for_legal_process]=0,
									1,
									0
								)
							)',
			...
			switch ($_GET['report_type']) {
				...
				case 'one_month':
					...
					$account_model->addCondition('last_premium','>=',$this->api->previousMonth($this->api->today. " -1 MONTH"));
					$account_model->addCondition('last_paid_date','<',$this->api->previousMonth($this->api->today. " -1 MONTH"));
					break;
				case 'two_month':
					$account_model->addCondition('due_premium_count',2);
					// $account_model->addCondition('due_premium_count','<=',2);
