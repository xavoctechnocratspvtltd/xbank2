...
class page_stock_actions_move extends Page {
	function init(){
		...
		$colright->add('View')->set('To')->setStyle(array('background-color'=>'#e1e1e1','padding'=>'5px'));
			// TO Container
		$to_container_field = $form->addField('dropdown','to_container','Container')->setEmptyText('Please Select');
		$to_container_model = $this->add('Model_Stock_Container');	
			//Todo Load Container Accrding to branch  selection
			//$to_container_model->addCondition('branch_id',$form['to_branch']);
		$to_container_field->setModel($to_container_model);	
		$to_container_field->js(true)->closest('div.atk-form-row')->appendTo($colright);
			// To Row
		$to_row_field = $form->addField('dropdown','to_row','Row')->setEmptyText('Please Select');
