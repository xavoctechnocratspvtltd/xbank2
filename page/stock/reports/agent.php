...
class page_stock_reports_agent extends Page {
	function page_index(){
		// parent::init();
		...
		$v=$this->add('View');
		if($_GET['filter'])
			$view_consume=$v->add('View_StockMember_Report',array('member'=>$_GET['agent'],'item'=>$_GET['item'],'from_date'=>$_GET['from_date'],'to_date'=>$form['to_date'],'filter'=>$_GET['filter'],'type'=>'Staff'));
